...
@Component({
  tag: 'bce-option-old',
  styleUrl: 'bce-option.old.scss',
  shadow: false
})
export class BceOptionOld {
  @Element()
  private el!: HTMLBceOptionOldElement;
  ...
  private handleInput = (event: Event) => {
    ...
    if (this.type === 'radio' && this.parent) {
      this.parent
        .querySelectorAll('bce-option-old')
    ...
    switch (this.type) {
      case 'checkbox': {
        const options = this.parent.querySelectorAll('bce-option-old');
