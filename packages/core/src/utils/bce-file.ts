export class BceFile {
  ...
  public hash(algorithm?: string, output?: 'hex'): Promise<string>;
  public hash(algorithm?: string, output?: string): Promise<number[]>;
  public async hash(algorithm = 'SHA-256', output = 'hex') {
    if (!this.#digest || this.#digestAlg !== algorithm) {
      const data = await new Response(this.blob).arrayBuffer();
